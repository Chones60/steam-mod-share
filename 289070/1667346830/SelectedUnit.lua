...
--	VARIABLES
-- ===========================================================================
local m_isDisableWaterAvailLens:boolean = true;
local m_isDisableReligionLens:boolean = true;
...
function OnUnitSelectionChanged( playerID:number, unitID:number, hexI:number, hexJ:number, hexK:number, isSelected:boolean, isEditable:boolean )
	...
	if pPlayer ~= nil then
		...
		if isSelected then
			...
			if religiousStrength > 0 and not UILens.IsLensActive("Religion") and (not m_isDisableReligionLens) then
